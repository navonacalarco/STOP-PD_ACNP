...
df$diffAverageFA_MD <- df$AverageFA_MD.02 - df$AverageFA_MD.01
df$diffSLF_MD <- df$SLF_MD.02 - df$SLF_MD.01
df$diffSLF.L_MD <- df$SLF.L_MD.02 - df$SLF.L_MD.01
df$diffSLF.R_MD <- df$SLF.R_MD.02 - df$SLF.R_MD.01
...
#for now, just review SLF_L

#make sure relationship is linear
plot_linear <- scatter.smooth(x=df_rct$SLF.L_MD.01, y= df_rct$SLF.L_MD.02) 

#check shape
par(mfrow=c(1, 2))
plot_01 <- boxplot(df_rct$SLF.L_MD.01, main = "timepoint 1") 
plot_02 <- boxplot(df_rct$SLF.L_MD.02, main = "timepoint 2") 

#check density, i.e., if close to normality
par(mfrow=c(1, 2))
plot(density(df_rct$SLF.L_MD.01), main = "timepoint 1") 
plot(density(df_rct$SLF.L_MD.02), main = "timepoint 2") 

#check correlation
cor(df_rct$SLF.L_MD.01, df_rct$SLF.L_MD.02) 
...
plot <- ggplot(df_rct, aes(x= randomization, y = diffSLF.L_MD)) + geom_boxplot() + geom_jitter() + labs(y = 'difference')
plot

#run linear model without covariates
fit_rct <- lm(diffSLF.L_MD ~ randomization + (1|id), data= df_rct)
print(fit_rct)
summary(fit_rct)

#run linear model with covariates of sex and age
fit_rct <- lm(diffSLF.L_MD ~ randomization + Sex + Age + (1|id), data= df_rct)
print(fit_rct)
summary(fit_rct)
...
#make a subset of data for just l_SLF
l_SLF <- mdf[(mdf$metric == 'SLF.L'),]
...
fit_all <- lmer(metric_t ~ randomization*model_days + Sex + Age + (1|id), data= l_SLF)
summary(fit_all)

```

## RCT & Relapse (with time as factor) - average MD

```{r all_AverageFA, include = TRUE}

#make a subset of data for just Average FA (i.e., average MD)
AverageFA <- mdf[(mdf$metric == 'AverageFA'),]
...
#run mixed linear model, with covariates
fit_all <- lmer(metric_t ~ randomization*model_days + Sex + Age + (1|id), data= AverageFA )
summary(fit_all)
