...
plot_linear <- scatter.smooth(x=df_rct$AverageFA_MD.01, y= df_rct$AverageFA_MD.02) 

#check shape
par(mfrow=c(1, 2))
plot_01 <- boxplot(df_rct$AverageFA_MD.01, main = "timepoint 1") 
plot_02 <- boxplot(df_rct$AverageFA_MD.02, main = "timepoint 2") 

#check density, i.e., if close to normality
par(mfrow=c(1, 2))
plot(density(df_rct$AverageFA_MD.01), main = "timepoint 1") 
plot(density(df_rct$AverageFA_MD.02), main = "timepoint 2") 

#check correlation
cor(df_rct$AverageFA_MD.01, df_rct$AverageFA_MD.02) 
...
plot <- ggplot(df_rct, aes(x= randomization, y = diffAverageFA_MD)) + geom_boxplot() + geom_jitter() + labs(y = 'difference')
plot

#run linear model without covariates
fit_rct <- lm(diffAverageFA_MD ~ randomization + (1|id), data= df_rct)
print(fit_rct)
summary(fit_rct)

#run linear model with covariates of sex and age
fit_rct <- lm(diffAverageFA_MD ~ randomization + Sex + Age + (1|id), data= df_rct)
print(fit_rct)
summary(fit_rct)
...
#make a subset of data for just whole skeleton MD
MD <- mdf[(mdf$metric == 'AverageFA'),]
...
fit_all <- lmer(metric_t ~ randomization*model_days + Sex + Age + (1|id), data= MD)
summary(fit_all)
